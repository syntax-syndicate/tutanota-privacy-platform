...
export class KeyRotationFacade {
	 ...
	 * @param user
	 */
	async processPendingKeyRotations(user: User, pwKey: Aes256Key | null): Promise<void> {
		try {
...
export class KeyRotationRolloutAction implements RolloutAction {
	...
	public async execute() {
		...
		if (user && user.accountType !== AccountType.EXTERNAL) {
			...
			await this.keyRotationFacade.processPendingKeyRotations(user, requiredPasswordKey)
		}
	}
}
